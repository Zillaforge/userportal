...
const hpcRemoteImageList: Ref<Record<string, any>[]> = ref([]);
...
const fetchData = async (showLoading: boolean = true) => {
  ...
  hpcRemoteImageList.value = await makeApiCall({
...
const getTdItem = (item: Record<string, any>, header: string) => {
  if (header === 'size') {
    ...
    const imageUri: string = item?.uri || '';
    if (imageUri) {
      const subUrl = imageUri.split('/');
...
const toCreatePage = async () =>
  await router.push({ name: PAGE_TYPES.HPC_REMOTE_IMAGE_CREATE });
</script>
      ...
      <GeneralDataTable
        ...
        :more-action-list="[
          ...
          },
          {
            ...
            resourceType: $t('services.hpc.remote.image'),
